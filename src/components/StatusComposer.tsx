...
export function StatusComposer({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const newAtt of newAttachments) {
      ...
      const interval = setInterval(() => {
        ...
        setAttachments((prev) =>
          prev.map((a) =>
            a.file === newAtt.file ? { ...a, progress: simulatedProgress } : a
          )
        )
      }, 200)
      ...
      try {
        ...
        setAttachments((prev) =>
          prev.map((a) =>
            a.file === newAtt.file ? { ...a, progress: 100, error: true } : a
          )
        )
      }
    }
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            size="icon"
            aria-label={label}
            className={`h-8 w-8 ${currentVisibility === value ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"}`}
  ...
  return (
    <Card
      className={`w-full border-2 border-primary/10 shadow-sm transition-all focus-within:border-primary/30 ${className}`}
    >
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit(onSubmit)} className="flex gap-4">
          ...
          <div className="min-w-0 flex-1 space-y-3">
            {quoteId && (
              <div className="flex items-center gap-2 rounded-r-md border-l-4 border-l-primary/50 bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
            ...
            <Controller
              ...
              render={({ field }) => (
                <Textarea
                  ...
                  placeholder={
                    showCW
                      ? "Type your hidden message here..."
                      : "What's on your mind?"
                  }
                  aria-label={
                    showCW ? "Type your hidden message here" : "Status content"
                  }
            ...
            {attachments.length > 0 && (
              <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-4">
                {attachments.map((att, i) => (
                  <div
                    key={i}
                    className="group relative flex aspect-video items-center justify-center overflow-hidden rounded-md bg-muted"
                  >
                    <img
                      src={att.preview}
                      alt="Upload preview"
                    ...
                    {att.progress < 100 && (
                      <div className="absolute inset-0 flex items-center justify-center bg-background/50 p-2">
                        <Progress value={att.progress} className="w-full" />
                      </div>
                    )}

                    {att.error && (
                      <div className="absolute inset-0 flex items-center justify-center bg-red-500/20 text-sm font-bold text-red-500">
                        Failed
                      </div>
                    )}
                    ...
                    <Button
                      ...
                      className="absolute top-1 right-1 h-6 w-6 scale-90 opacity-0 transition-opacity group-hover:opacity-100"
                    ...
                    <TooltipTrigger
                      render={
                        <Button
                          ...
                          className={`h-8 w-8 ${currentLocalOnly ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"}`}
                          onClick={() =>
                            setValue("localOnly", !currentLocalOnly)
                          }
                        >
                    ...
                    render={({ field }) => (
                      ...
                      >
                        <SelectTrigger className="h-8 w-[110px] text-xs">
                          {contentTypes.find((c) => c.value === field.value)
                            ?.label || "Format"}
                        </SelectTrigger>
                        <SelectContent>
                          {contentTypes.map((c) => (
            ...
            <TooltipProvider>
              <div className="flex items-center justify-between pt-1">
                    ...
                    <TooltipTrigger
                      render={
                        <Button
                          ...
                          onClick={() =>
                            setValue("poll.show", !currentPollShow)
                          }
                        >
                ...
                <div className="flex items-center space-x-4">
                  <span
                    className={`text-xs font-medium ${isOverLimit ? "text-red-500" : "text-muted-foreground/60"}`}
                  >
                    {remainingChars}
                  </span>
